 ...
 *                                        (https://<project-ref>.supabase.co/functions/v1/anonymous-feedback-intake)
 *   ANONYMOUS_FEEDBACK_WEBHOOK_SECRET   o MESMO segredo configurado na Edge
 *                                        Function — EXCLUSIVO desta
 *                                        integração. NUNCA reaproveite
 *                                        GOOGLE_FORMS_WEBHOOK_SECRET (member
 *                                        intake) nem qualquer chave de CPF.
 *   ALLOWED_FORM_ID                      o ID deste formulário (Arquivo >
 *                                        Detalhes, ou
...
function getConfig_() {
  ...
  var webhookSecret = props.getProperty('ANONYMOUS_FEEDBACK_WEBHOOK_SECRET');
  var allowedFormId = props.getProperty('ALLOWED_FORM_ID');

  var missing = [];
  if (!webhookUrl) missing.push('WEBHOOK_URL');
  if (!webhookSecret) missing.push('ANONYMOUS_FEEDBACK_WEBHOOK_SECRET');
