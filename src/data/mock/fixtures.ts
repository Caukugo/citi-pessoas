...
const SEED_X1S: SeedX1[] = [
  // Helena — em dia, com histórico
  {
    ...
    citiValues: [
      {
        valueId: CITI_VALUE_IDS.euSouOCiti,
        value: 'Eu sou o CITi',
        rating: 4,
        note: 'Muito presente na cultura da squad.',
      },
      {
        valueId: CITI_VALUE_IDS.aprender,
        value: 'Obcecados por aprender',
        rating: 4,
        note: 'Estudou TypeScript avançado por conta própria.',
      },
      { valueId: CITI_VALUE_IDS.vencer, value: 'Obcecados por vencer', rating: 4, note: null },
      {
        valueId: CITI_VALUE_IDS.entregar,
        value: 'Obcecados por entregar',
        rating: 4,
        note: 'Cumpriu todos os prazos do ciclo.',
      },
    ],
...
/**
 * Um instante a partir de dia + hora de parede em Recife.
 *
 * Recife é UTC−3 o ano inteiro (não tem horário de verão), então o offset fixo
 * é exato aqui. O código de produto NÃO faz isso: ele usa
 * `zonedTimeToInstant()` em `@/features/x1/model/timeZone`, que funciona para
 * qualquer fuso.
 */
function atRecife(day: string, time: string): string {
  return new Date(`${day}T${time}:00-03:00`).toISOString();
}

type SeedAppointment = Omit<X1Appointment, 'createdAt' | 'updatedAt'>;

/**
 * Os compromissos de exemplo cobrem os estados que a tela precisa tratar —
 * inclusive os feios. Não é uma agenda bonita de propósito: é uma agenda com
 * Meet pendente, sincronização falhada e legado sem horário, porque é aí que
 * os bugs moram.
 *
 * `prf-001` = Marina Quintela (usr-001) · `prf-002` = Otávio Bandeira (usr-002).
 * No mock o id do profile é o do usuário — no Supabase, `profiles.id` é o
 * `auth.users.id`.
 */
const SEED_X1_APPOINTMENTS: SeedAppointment[] = [
  // Hoje, 14:00 — aceito, sincronizado, Meet pronto. O caso feliz.
  {
    id: 'apt-001',
    memberId: 'mbr-010',
    organizerProfileId: 'usr-001',
    conductedById: 'mbr-001',
    startsAt: atRecife(daysAgo(0), '14:00'),
    endsAt: atRecife(daysAgo(0), '15:00'),
    scheduledDate: null,
    durationMinutes: 60,
    ...
    inviteResponseAt: atRecife(daysAgo(2), '09:12'),
    syncStatus: 'sincronizado',
    title: null,
    sharedAgenda: 'Acompanhamento do mês e próximos passos.',
    internalNotes: 'Conferir como ela está lidando com o acúmulo de squad.',
    ...
    gestaoId: CURRENT_GESTAO_ID,
    versao: 0,
    createdByProfileId: 'usr-001',
    updatedByProfileId: null,
    event: {
      calendarId: 'primary',
      eventId: 'a1b2c3d4e5f6g7h8',
      etag: '"mock-etag-001"',
      htmlLink: 'https://calendar.google.com/calendar/event?eid=exemplo-001',
      hangoutLink: 'https://meet.google.com/abc-defg-hij',
      meetStatus: 'disponivel',
      invitedEmail: 'bernadete.siqueira@citi.org.br',
      lastSyncedAt: NOW,
    },
  },

  // Hoje, 16:00 — presencial, ainda sem resposta.
  {
    id: 'apt-002',
    memberId: 'mbr-009',
    organizerProfileId: 'usr-001',
    conductedById: 'mbr-001',
    startsAt: atRecife(daysAgo(0), '16:00'),
    endsAt: atRecife(daysAgo(0), '16:45'),
    scheduledDate: null,
    durationMinutes: 45,
    timeZone: 'America/Recife',
    mode: 'presencial',
    location: 'Sala de reuniões · CITi',
    wantsMeet: false,
    status: 'agendado',
    ...
    syncStatus: 'sincronizado',
    title: null,
    sharedAgenda: 'Primeira conversa de acompanhamento.',
    internalNotes: null,
    cancellationReason: null,
    cancelledAt: null,
    cancelledByProfileId: null,
    ...
    event: {
      calendarId: 'primary',
      eventId: 'b2c3d4e5f6g7h8i9',
      etag: '"mock-etag-002"',
      htmlLink: 'https://calendar.google.com/calendar/event?eid=exemplo-002',
      hangoutLink: null,
      meetStatus: 'sem_meet',
      invitedEmail: 'edmundo.vilanova@citi.org.br',
      lastSyncedAt: NOW,
    },
  },

  // Organizado por OUTRA pessoa de GG. Exercita "Toda GG" e a regra de que só
  // o organizador reagenda ou cancela.
  {
    id: 'apt-003',
    memberId: 'mbr-006',
    organizerProfileId: 'usr-002',
    conductedById: 'mbr-002',
    startsAt: atRecife(daysAhead(2), '10:00'),
    endsAt: atRecife(daysAhead(2), '11:00'),
    scheduledDate: null,
    durationMinutes: 60,
    timeZone: 'America/Recife',
    mode: 'online',
    ...
    wantsMeet: true,
    status: 'agendado',
    inviteResponse: 'talvez',
    inviteResponseAt: atRecife(daysAgo(1), '18:40'),
    syncStatus: 'sincronizado',
    title: null,
    sharedAgenda: 'Acompanhamento mensal.',
    internalNotes: null,
    cancellationReason: null,
    cancelledAt: null,
    cancelledByProfileId: null,
    ...
    gestaoId: CURRENT_GESTAO_ID,
    versao: 0,
    createdByProfileId: 'usr-002',
    updatedByProfileId: null,
    event: {
      calendarId: 'primary',
      eventId: 'c3d4e5f6g7h8i9j0',
      etag: '"mock-etag-003"',
      htmlLink: 'https://calendar.google.com/calendar/event?eid=exemplo-003',
      hangoutLink: 'https://meet.google.com/klm-nopq-rst',
      meetStatus: 'disponivel',
      invitedEmail: 'iris.cavalcanti@citi.org.br',
      lastSyncedAt: NOW,
    },
  },

  // ⚠️ Terminou e ninguém registrou: "Aguardando registro".
  // Isso NÃO é falta e NÃO muda a situação do membro — é pendência operacional.
  {
    id: 'apt-004',
    memberId: 'mbr-007',
    organizerProfileId: 'usr-001',
    conductedById: 'mbr-001',
    startsAt: atRecife(daysAgo(3), '14:00'),
    endsAt: atRecife(daysAgo(3), '15:00'),
    scheduledDate: null,
    durationMinutes: 60,
    timeZone: 'America/Recife',
    mode: 'online',
    ...
    status: 'agendado',
    inviteResponse: 'aceito',
    inviteResponseAt: atRecife(daysAgo(5), '11:00'),
    syncStatus: 'sincronizado',
    title: null,
    sharedAgenda: null,
    internalNotes: null,
    ...
    event: {
      calendarId: 'primary',
      eventId: 'd4e5f6g7h8i9j0k1',
      etag: '"mock-etag-004"',
      htmlLink: 'https://calendar.google.com/calendar/event?eid=exemplo-004',
      hangoutLink: 'https://meet.google.com/uvw-xyza-bcd',
      meetStatus: 'disponivel',
      invitedEmail: 'wagner.beltrao@citi.org.br',
      lastSyncedAt: NOW,
    },
  },

  // ⚠️ Alteração pendente: o Google não confirmou. O horário abaixo é o ÚLTIMO
  // CONFIRMADO, não o que foi pedido — a tela precisa mostrar os dois.
  {
    id: 'apt-005',
    memberId: 'mbr-008',
    organizerProfileId: 'usr-001',
    conductedById: 'mbr-001',
    startsAt: atRecife(daysAhead(1), '09:00'),
    endsAt: atRecife(daysAhead(1), '10:00'),
    scheduledDate: null,
    durationMinutes: 60,
    timeZone: 'America/Recife',
    mode: 'online',
    ...
    syncStatus: 'falha',
    title: null,
    sharedAgenda: 'Alinhamento de expectativas.',
    internalNotes: null,
    cancellationReason: null,
    cancelledAt: null,
    cancelledByProfileId: null,
    ...
    versao: 1,
    createdByProfileId: 'usr-001',
    updatedByProfileId: 'usr-001',
    event: {
      calendarId: 'primary',
      eventId: 'e5f6g7h8i9j0k1l2',
      etag: '"mock-etag-005"',
      htmlLink: 'https://calendar.google.com/calendar/event?eid=exemplo-005',
      hangoutLink: null,
      meetStatus: 'sem_meet',
      invitedEmail: 'solange.peixoto@citi.org.br',
      lastSyncedAt: atRecife(daysAgo(1), '13:42'),
    },
  },

  // ⚠️ Meet ainda em geração. "Pedir não é ter": a tela diz que está pendente
  // em vez de mostrar um link que não existe.
  {
    id: 'apt-006',
    memberId: 'mbr-004',
    organizerProfileId: 'usr-001',
    conductedById: 'mbr-001',
    startsAt: atRecife(daysAhead(5), '15:00'),
    endsAt: atRecife(daysAhead(5), '15:30'),
    scheduledDate: null,
    durationMinutes: 30,
    timeZone: 'America/Recife',
    mode: 'online',
    ...
    event: {
      calendarId: 'primary',
      eventId: 'f6g7h8i9j0k1l2m3',
      etag: '"mock-etag-006"',
      htmlLink: 'https://calendar.google.com/calendar/event?eid=exemplo-006',
      hangoutLink: null,
      meetStatus: 'pendente',
      invitedEmail: 'ricardo.tenorio@citi.org.br',
      lastSyncedAt: NOW,
    },
  },

  // Cancelado. Some da agenda por padrão, mas o histórico fica.
  {
    id: 'apt-007',
    memberId: 'mbr-011',
    organizerProfileId: 'usr-001',
    conductedById: 'mbr-001',
    startsAt: atRecife(daysAhead(3), '11:00'),
    endsAt: atRecife(daysAhead(3), '12:00'),
    scheduledDate: null,
    durationMinutes: 60,
    timeZone: 'America/Recife',
    mode: 'online',
    ...
    wantsMeet: true,
    status: 'cancelado',
    inviteResponse: 'recusado',
    inviteResponseAt: atRecife(daysAgo(2), '20:15'),
    syncStatus: 'sincronizado',
    title: null,
    sharedAgenda: null,
    internalNotes: null,
    // ⚠️ Nunca foi ao Google. Só GG vê isto.
    cancellationReason: 'Conflito com a entrega do squad; remarcar na semana que vem.',
    cancelledAt: atRecife(daysAgo(1), '08:30'),
    cancelledByProfileId: 'usr-001',
    x1Id: null,
    origin: 'plataforma',
    originX1Id: null,
    gestaoId: CURRENT_GESTAO_ID,
...
export const X1_APPOINTMENTS: X1Appointment[] = SEED_X1_APPOINTMENTS.map((appointment) => ({
  ...appointment,
  createdAt: NOW,
  updatedAt: NOW,
}));
